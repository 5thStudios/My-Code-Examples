...
{
    public class SearchScholarsController : RenderController
    {
        private IPublishedValueFallback _publishedValueFallback;
        private readonly IExamineManager ExamineManager;
        private UmbracoHelper UmbracoHelper;
        private readonly ILogger<SearchScholarsController> logger;
        private readonly IPublishedContentQuery publishedContentQuery;

        public SearchScholarsController(
                ILogger<SearchScholarsController> _logger,
        ...
        public override IActionResult Index()
        {
            ...
            var cmPage = new ContentModels.SearchScholars(CurrentPage, _publishedValueFallback);
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(query))
                {
                    if (ExamineManager.TryGetIndex(UmbracoIndexes.ExternalIndexName, out IIndex? index))
                    {
                        ...
                        lstExcludedAliases.Add(Common.Doctype.SearchQTQ);
                        lstExcludedAliases.Add(Common.Doctype.DocumentCardQTQ);
                        lstExcludedAliases.Add(Common.Doctype.IconCardCompQTQ);
                        lstExcludedAliases.Add(Common.Doctype.PartnerCardQTQ);
            ...
            var viewModel = new ComposedPageViewModel<ContentModels.SearchScholars, List<SearchResultViewModel>>
            {
                Page = cmPage,
                ViewModel = lstSearchResults
            };

            return View(Common.View.SearchScholars, viewModel);
        }
    }
}
